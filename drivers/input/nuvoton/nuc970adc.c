...
static int nuc970adc_battery_read_adc(struct nuc970_adc *nuc970_adc)
{
	...
	{
		enable_menu();
		while((__raw_readl(REG_ADC_ISR) & (ADC_ISR_VBF|ADC_ISR_MF))!=(ADC_ISR_VBF|ADC_ISR_MF)) mdelay(1);
...
static int nuc970adc_probe(struct platform_device *pdev)
{
  ...
  __raw_writel(0x03000000,REG_CLK_DIV7);
	...
	input_kp_dev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_REP) | BIT_MASK(EV_MSC); // | BIT_MASK(EV_SND) ;// | BIT_MASK(EV_REP);
...
fail5:	free_irq(nuc970_adc->irq_num, nuc970_adc);
fail4:	clk_put(nuc970_adc->clk);
fail1:	if(!input_ts_dev) input_free_device(input_ts_dev);
