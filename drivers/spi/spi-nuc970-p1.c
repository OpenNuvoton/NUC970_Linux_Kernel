...
static inline void nuc970_spi1_chipsel(struct spi_device *spi, int value)
{
	switch (value) {
...
static inline void nuc970_spi1_setup_txnum(struct nuc970_spi *hw,
							unsigned int txnum)
{
	unsigned int val;
	unsigned long flags;
...
static inline void nuc970_spi1_setup_txbitlen(struct nuc970_spi *hw,
...
#ifdef CONFIG_OF
static struct nuc970_spi_info *nuc970_spi1_parse_dt(struct device *dev)
{
...
static int nuc970_spi1_probe(struct platform_device *pdev)
{
	...
	hw->pdata = nuc970_spi1_parse_dt(&pdev->dev);
	hw->dev = &pdev->dev;
...
#if defined(CONFIG_OF)
static const struct of_device_id nuc970_spi1_of_match[] = {
	{   .compatible = "nuvoton,nuc970-spi1"},
	{  },
};
MODULE_DEVICE_TABLE(of, nuc970_spi1_of_match);
#endif
...
#if defined(CONFIG_OF)
		.of_match_table = of_match_ptr(nuc970_spi1_of_match),
#endif
	},
};
...
MODULE_DESCRIPTION("NUC970/N9H30 SPI1 driver");
MODULE_LICENSE("GPL");
