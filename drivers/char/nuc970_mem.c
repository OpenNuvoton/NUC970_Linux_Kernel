...
long nuc970_mem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case NUC970_MEM_ALLOC:
		...
		param.vir_addr = do_mmap_pgoff(file, 0, param.size,
				(PROT_READ|PROT_WRITE), MAP_SHARED, 0, &rsize);
		DEBUG("param.vir_addr = %08x, %d\n",
						param.vir_addr, __LINE__);
		if (param.vir_addr == -EINVAL) {
			printk(KERN_INFO "NUC970_MEM_ALLOC FAILED\n");
			flag = 0;
			mutex_unlock(&mem_alloc_lock);
			return -EFAULT;
		}
	...
	case NUC970_MEM_CACHEABLE_ALLOC:
	...
	case NUC970_MEM_SHARE_ALLOC:
		...
		DEBUG("param.vir_addr = %08x, %d\n",
				param.vir_addr, __LINE__);
		if (param.vir_addr == -EINVAL) {
			printk(KERN_INFO "NUC970_MEM_SHARE_ALLOC FAILED\n");
			flag = 0;
			mutex_unlock(&mem_share_alloc_lock);
			return -EFAULT;
		}
	...
	case NUC970_MEM_CACHEABLE_SHARE_ALLOC:
	...
	default:
		DEBUG("nuc970_mem_ioctl() : default !!\n");
		return -EINVAL;
	}

	return 0;
}
EXPORT_SYMBOL(nuc970_mem_ioctl);
